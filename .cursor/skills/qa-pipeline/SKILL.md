...
User provides a test case document OR asks to "run the pipeline", "run full pipeline", or "end-to-end".
...
**Agent routing:** Exclusive routing (first match wins) is defined in **`sdlc-core.mdc`**. **Disambiguation:** *"Automate my tests"* → `@test-generator`. *"Run pipeline on my document"* → `@qa-pipeline`.
...
- [ ] Headed MCP login completed OR user explicitly skips auth scenarios per [LOGIN.md](../../system-context/LOGIN.md)
...
See [LOGIN.md](../../system-context/LOGIN.md). OTP is **never** automated — user provides in chat during stage 2.
...
Every feature: `@regression`, `@positive`, `@negative`, `@edge` per [APPLICATION.md](../../system-context/APPLICATION.md).
